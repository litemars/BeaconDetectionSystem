...
class SyslogHandler:
    ...
    def _setup_syslog(self):
        ...
        facility = facility_map.get(
            self.config.syslog_facility, logging.handlers.SysLogHandler.LOG_LOCAL0
        )
        ...
        try:
            ...
            if os.path.exists(self.config.syslog_address):
                handler = logging.handlers.SysLogHandler(
                    address=self.config.syslog_address, facility=facility
                )
            else:
            ...
            formatter = logging.Formatter("beacon-detect[%(process)d]: %(message)s")
...
class WebhookHandler:
    ...
    def _setup_session(self):
        ...
        # Set default headers
        headers = {"Content-Type": "application/json", "User-Agent": "BeaconDetect/1.0"}
    ...
    def _send_request(self, payload: str):
        ...
        response = self._session.post(
            self.config.webhook_url, data=payload, timeout=self.config.webhook_timeout
        )
