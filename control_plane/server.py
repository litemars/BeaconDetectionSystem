...
class ControlPlaneServer:
    ...
    def _init_alerter(self, config):
        ...
        alerting_config = AlertingConfig(
            ...
            file_path=alert_config.get("file", {}).get(
                "path", "/var/log/beacon-detect/alerts.json"
            ),
    ...
    async def _handle_telemetry(self, request: web.Request) -> web.Response:
        ...
        try:
            ...
            if "events" not in data:
                return web.json_response({"error": "Missing events field"}, status=400)
    ...
    async def _handle_beacons(self, request: web.Request) -> web.Response:
        ...
        return web.json_response(
            {"count": len(beacons), "beacons": [b.to_dict() for b in beacons]}
        )
    ...
    async def _handle_alerts(self, request: web.Request) -> web.Response:
        ...
        return web.json_response({"count": len(alerts), "alerts": alerts})
                ...
                "pairs": [
                    {
                        ...
                        "first_seen": (
                            datetime.fromtimestamp(p.first_seen).isoformat() + "Z"
                            if p.first_seen
                            else None
                        ),
                        "last_seen": (
                            datetime.fromtimestamp(p.last_seen).isoformat() + "Z"
                            if p.last_seen
                            else None
                        ),
                    }
        ...
        try:
            ...
            if "detection" in data:
                ...
                if hasattr(self.analyzer, "config"):
                    if "analysis_interval" in det:
                        self.analyzer.config.analysis_interval = det.get(
                            "analysis_interval", 60
                        )
                    if "alert_cooldown" in det:
                        self.analyzer.config.alert_cooldown = det.get(
                            "alert_cooldown", 300
                        )
            ...
            if "whitelist" in data:
                ...
                if "source_ips" in data["whitelist"]:
                    self.config.setdefault("whitelist", {})["source_ips"] = data[
                        "whitelist"
                    ]["source_ips"]
                if "destination_ips" in data["whitelist"]:
                    self.config.setdefault("whitelist", {})["destination_ips"] = data[
                        "whitelist"
                    ]["destination_ips"]
                if "destination_ports" in data["whitelist"]:
                    self.config.setdefault("whitelist", {})["ports"] = data[
                        "whitelist"
                    ]["destination_ports"]
