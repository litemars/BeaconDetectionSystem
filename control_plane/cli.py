...
/_____/\___/\__,_/\___/\____/_/ /_//_____/\___/\__/\___/\___/\__/  
    ...
    def print_table(self, headers, rows, col_widths=None):
        ...
        for i, h in enumerate(headers):
            header_line += c(
                h.ljust(col_widths[i] + 2), Colors.BOLD, Colors.BRIGHT_CYAN
            )
        print(header_line)
    ...
    def cmd_status(self):
        ...
        stats = [
            ...
            (
                "Alerts Generated",
                f"{status.get('alerter', {}).get('alerts_sent', 0):,}",
            ),
            (
                "Analysis Runs",
    ...
    def cmd_beacons(
        self, min_score: float = 0.0, limit: int = 50, csv_output: bool = False
    ):
        ...
        if csv_output:
            # CSV output
            print(
                "Score,Severity,Source_IP,Source_Port,Dest_IP,Dest_Port,Protocol,Connections,Interval,Jitter"
            )
            for b in beacons:
                ...
                severity = (
                    "CRITICAL"
                    if score >= 0.9
                    else "HIGH" if score >= 0.8 else "MEDIUM" if score >= 0.7 else "LOW"
                )
                interval = b.get("interval_stats", {}).get("mean", 0)
                jitter = b.get("interval_stats", {}).get("jitter", 0)
                print(
                    f"{score:.4f},{severity},{b.get('src_ip', '-')},-,{b.get('dst_ip', '-')},{b.get('dst_port', '-')},{b.get('protocol', 'TCP')},{b.get('connection_count', 0)},{interval:.2f},{jitter:.2f}"
                )
        else:
            # Table output
            print(
                f"  Found {c(str(len(beacons)), Colors.BOLD)} beacon(s) with score >= {min_score:.1%}"
            )
            print()
    ...
    def cmd_long_connections(self, min_duration=3600, limit=50, csv_output=False):
        ...
        if not pairs:
            if not csv_output:
                print(
                    c(
                        f"  No connections longer than {self.format_duration(min_duration)}",
                        Colors.DIM,
                    )
                )
                print()
        ...
        if csv_output:
            print(
                "Duration,Source_IP,Dest_IP,Dest_Port,Protocol,Connections,First_Seen,Last_Seen"
            )
            for p in pairs:
                duration = p.get("duration_seconds", 0)
                print(
                    f"{duration:.0f},{p.get('src_ip', '-')},{p.get('dst_ip', '-')},{p.get('dst_port', '-')},{p.get('protocol', 'TCP')},{p.get('connection_count', 0)},{p.get('first_seen', '-')},{p.get('last_seen', '-')}"
                )
        else:
            print(
                f"  Found {c(str(len(pairs)), Colors.BOLD)} connection(s) longer than {self.format_duration(min_duration)}"
            )
            print()
    ...
    def cmd_connections(self, limit: int = 100, csv_output: bool = False):
        ...
        if csv_output:
            print(
                "Source_IP,Dest_IP,Dest_Port,Protocol,Connections,Duration,First_Seen,Last_Seen"
            )
            for p in pairs:
                print(
                    f"{p.get('src_ip', '-')},{p.get('dst_ip', '-')},{p.get('dst_port', '-')},{p.get('protocol', 'TCP')},{p.get('connection_count', 0)},{p.get('duration_seconds', 0):.0f},{p.get('first_seen', '-')},{p.get('last_seen', '-')}"
                )
        else:
        ...
        try:
            while True:
                ...
                if not health or not status:
                    ...
                    print(
                        c(
                            f"    Waiting for connection to {self.base_url}...",
                            Colors.DIM,
                        )
                    )
                else:
                    ...
                    print(
                        f"  {c('●', Colors.BRIGHT_GREEN)} {c('LIVE', Colors.BOLD, Colors.BRIGHT_GREEN)}  │  "
                    ...
                    if data:
                        ...
                        if beacons:
                            ...
                            for b in beacons[:15]:
                                score = b.get("combined_score", 0)
                                interval_val = b.get("interval_stats", {}).get(
                                    "mean", 0
                                )
                                dest = (
                                    f"{b.get('dst_ip', '-')}:{b.get('dst_port', '-')}"
                                )
                            ...
                            print(
                                c("  No high-confidence beacons detected", Colors.DIM)
                            )
                    ...
                    if data:
                        long_conns = [
                            ...
                            if p.get("duration_seconds", 0) >= 3600
                        ]
                        long_conns.sort(
                            key=lambda x: x.get("duration_seconds", 0), reverse=True
                        )
                ...
                print(
                    c(
                        f"  Refreshing every {interval}s... Press Ctrl+C to exit",
                        Colors.DIM,
                    )
                )
                time.sleep(interval)
...
Examples:
  ...
  %(prog)s watch --interval 10             Refresh every 10 seconds
    ...
    parser.add_argument(
        "--host", default="localhost", help="Control plane host (default: localhost)"
    )
    parser.add_argument(
        "--port", type=int, default=9090, help="Control plane port (default: 9090)"
    )
    parser.add_argument(
    ...
    beacons_parser.add_argument(
        "--min-score", type=float, default=0.0, help="Minimum beacon score (0.0-1.0)"
    )
    beacons_parser.add_argument(
        "--limit", type=int, default=50, help="Maximum results to show"
    )
    beacons_parser.add_argument(
        "--csv", "-o", action="store_true", help="Output as CSV"
    )
    ...
    long_parser.add_argument("--csv", "-o", action="store_true", help="Output as CSV")

    # Connections command
    conns_parser = subparsers.add_parser("connections", help="Show all connections")
    conns_parser.add_argument(
        "--limit", type=int, default=100, help="Maximum results to show"
    )
    conns_parser.add_argument("--csv", "-o", action="store_true", help="Output as CSV")
    ...
    if args.command == "status" or args.command is None:
        ...
        cli.cmd_long_connections(
            min_duration=args.min_duration, limit=args.limit, csv_output=args.csv
        )
