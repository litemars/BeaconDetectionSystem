...
class PeriodicityResult:
    ...
    def to_dict(self):
        return {
            ...
            'dominant_period': float(round(self.dominant_period, 3)) if self.dominant_period else None,
            'periodicity_score': float(round(self.periodicity_score, 4)),
            'frequency_peaks': [
                (float(round(f, 6)), float(round(m, 4))) for f, m in self.frequency_peaks
            ]
        }
...
class DetectionResult:
    ...
    first_seen: str
    last_seen: str
    analysis_time: str = field(default_factory=lambda: datetime.now(timezone.utc).isoformat().replace('+00:00', 'Z'))
...
class BeaconDetector:
    ...
    def __init__(self, config = None):
        ...
        # Validate weights
        total_weight = (self.config.cv_weight + 
    ...
    def analyze(self, pair: ConnectionPair):
        ...
        result = DetectionResult(
            ...
            first_seen=datetime.fromtimestamp(pair.first_seen).isoformat() + 'Z' if pair.first_seen else '',
            last_seen=datetime.fromtimestamp(pair.last_seen).isoformat() + 'Z' if pair.last_seen else ''
        )
