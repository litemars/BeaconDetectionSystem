...
class ExporterConfig:
    ...
    def health_check_url(self):
        return (
            f"http://{self.control_plane_host}:{self.control_plane_port}/api/v1/health"
        )
...
class TelemetryExporter:
    ...
    async def start(self):
        ...
        connector = aiohttp.TCPConnector(
            limit=10, limit_per_host=5, keepalive_timeout=60
        )
        ...
        try:
            async with self._session.get(self.config.health_check_url) as response:
                if response.status == 200:
                    ...
                    self._control_plane_healthy = False
                    logger.warning(
                        f"Control plane health check failed: {response.status}"
                    )
                    return False
    ...
    async def export_batch(self, batch):
        ...
        try:
            async with self._session.post(
                self.config.control_plane_url, data=payload_bytes, headers=headers
            ) as response:
                if response.status == 200:
    ...
    async def export_events_chunked(self, events: list, chunk_size: int = None):
